...
namespace Veriflow.Avalonia.ViewModels
{
    ...
    public partial class SyncViewModel : ObservableObject
    {
        ...
        {
             // Stub for SaveFileDialog / FolderPicker
             System.Diagnostics.Debug.WriteLine("SelectExportFolder needs StorageProvider implementation");
        }
        ...
        private async Task ExportBatch()
        {
            ...
            IsBusy = false;
            // Stub message box
             System.Diagnostics.Debug.WriteLine("Batch Export Complete!");
        }
                ...
                try
                {
                    ...
                    if (isVideo)
                    {
                        ...
                        mediaFile.EndSeconds = mediaFile.StartSeconds + durSec;
                         ...
                         Dispatcher.UIThread.Invoke(() => VideoPool.Add(mediaFile));
                    }
                    else
                    {
                        ...
                        mediaFile.Fps = fps;
                         ...
                         Dispatcher.UIThread.Invoke(() => AudioPool.Add(mediaFile));
                    }
                }
                catch { }
            }
        ...
        private async Task AutoSync()
        {
            ...
            // Dispatcher.UIThread.Invoke(() => Matches.Clear());
                ...
                foreach (var vid in videosToProcess)
                {
                    ...
                    if (candidates.Any())
                    {
                        ...
                        foreach (var aud in candidates)
                        {
                            ...
                            Dispatcher.UIThread.Invoke(() => Matches.Add(pair));
                        }
                    }
                    else
                    {
                        // No match found
                        Dispatcher.UIThread.Invoke(() => Matches.Add(new SyncPair { Video = vid, Status = "No Match" }));
                    }
        ...
        private async Task AutoSyncByWaveform()
        {
            ...
            try
            {
                ...
                await Parallel.ForEachAsync(videosToProcess, parallelOptions, async (vid, ct) =>
                {
                    ...
                    Dispatcher.UIThread.Invoke(() => ProgressValue = progress);
                });
